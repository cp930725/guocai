...
<table width="100%" border="0" cellspacing="1" cellpadding="4" class='table_b'>
 ...
 <!---------------------------------------------支付宝微信支付--------------------------------------------------------->
<?}else if($memberBank['bankId']==32||$memberBank['bankId']==34||$memberBank['bankId']==35){
?>
<!--左边栏body-->
<div class="content3 wjcont">
 ...
 <div class="body">
	...
	<h2><?=$memberBank['bankName']?>扫码支付：<span>充值编号：</span><?=$args[0]['rechargeId']?></h2><br>
	<ul>
	<li style="height:auto;">
	<table width="100%" border="0" cellspacing="1" cellpadding="3" class='table_b'>
	<tr style="border:0px;"><td align="center" style="border:0px;">
    
    <!--<font color="#FF0000">您可以直接通过手机扫描下方二维码进行支付</font>-->
    
     <?php if($memberBank['bankId']==32||$memberBank['bankId']==34||$memberBank['bankId']==35){ 
	 if($memberBank['bankId']==32)
	 {$typ=2;}elseif($memberBank['bankId']==34){$typ=4;}else{$typ=5;}
	$appid='2017041386';//扫码应用的APPID
    $back_url='http://'.$_SERVER['HTTP_HOST'].'/back_url.php';
	  $back_url=urlencode($back_url);
	 $imgurl='http://pay2.58eq.cn/pay/img.php?appid='.$appid.'&typ='.$typ.'&payno='.$args[0]['rechargeId'].'&money='.$args[0]['amount'];
	 $linkurl='http://pay2.58eq.cn/pay/pay.php?appid='.$appid.'&typ='.$typ.'&payno='.$args[0]['rechargeId'].'&money='.$args[0]['amount'].'&back_url='.$back_url;
	 ?>
     <li><!--<span>扫码付款：</span><span><img src="<?=$imgurl?>" width="250" height="250"  /></span>-->
</li>  <?php }?>
<!--<br>扫一扫我，向我付款，这样更方便更快捷哦-->
    
    </td><td style="border:0px;"><a href="<?=$linkurl?>" target=_blank style="color:#ffffff; font-size:18px"class='formNext222'>点这里去网页扫码付款</a></td></tr>
	</table>
	 </li>
	 </ul>
</div><br>
 <div class="chongzhi2">
 	<h3>扫码充值说明：</h3>
    <ul>
     <li style="color:#e00c0c">1、系统会对充值金额进行加密，然后给出加密后的充值金额，加密值在0.001-0.005元之间。</li>
     <li style="color:#e00c0c">2、请认真核对系统加密后的充值金额，与加密金额不符充值将无法到账，需联系客服。</li>
     <li style="color:#e00c0c">3、充值后如10分钟未到账，请联系客服，告知您的充值编号和您的充值金额及你的银行用户姓名。</li>
    </ul>
 </div>
   <div class="bank"></div>
  </div>
