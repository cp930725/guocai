	...
	function __construct($dsn, $user='', $password=''){
		...
		try{
			...
			if(!$x['isOnLine'] && $x['state']==1){
				echo "<meta charset='utf-8'><script>alert('对不起,您的账号在别处登陆,您被强迫下线!');window.location.href='/index.php/user/login'</script>";
				exit();
			}else if(!$x['isOnLine']){
				echo "<meta charset='utf-8'><script>alert('对不起,登陆超时或网络不稳定,请重新登陆!');window.location.href='/index.php/user/login'</script>";
				exit();
			}
		}catch(Exception $e){
		}
