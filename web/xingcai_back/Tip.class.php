	...
	public final function getTKTip(){
		...
		if($data=$this->getCol($sql)){
			...
			if($state==4){
				return array(
					'flag'=>true,
					'message'=>'提款失败！<div style="text-align:center;margin-top:10px;font-size:16px;color:#555;">原因：'.$info.'</div>'
				);
			}else{
				return array(
					'flag'=>true,
					'message'=>'提款成功！<div style="text-align:center;margin-top:10px;font-size:18px;color:#555;">提款金额：<font color="#FF0000">'.$amount.'</font> 元</div>'
				);	
			}///end state
		}
	}
	...
	public final function getCZTip(){
		...
		if($data=$this->getCol($sql)){
			...
			if($CZAmount>0){
				return array(
					'flag'=>true,
					'message'=>'充值成功！<div style="text-align:center;margin-top:10px;font-size:18px;color:#555;">到账金额：<font color="#FF0000">'.$CZAmount.'</font> 元</div>'
				);
			}else{
				return array(
					'flag'=>true,
