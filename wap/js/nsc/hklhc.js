...
function lhcPostData(){
	...
	try{
		...
		$val=$this.val(),
		//$sRte=$this.closest('dd').find('.num'),
		$sRte=$this.parents('td').prev().find('.num'),
		re=/^[1-9][0-9]*$/;
		
		//console.log($sRte);
		//console.error($sRte.text());
		if($val>0 && !isNaN($val)){
			if(!re.test($val)){
				//$.alert('只能为大于1正整数');
				xingcai('只能为大于1正整数');
			...
			newcode={
				...
				bonusPropName:$.trim($sRte.attr("data-id")),
				actionNum: 1,
	...
	$.ajax('/index.php/game/postlhcData', {
		...
		complete:function(xhr, textStatus){
			...
			if(errorMessage){
				gamePostedLHCData(decodeURIComponent(errorMessage));
				//destroyWait();
			}else{
				//
			}
		
			$(".Contentbox input").val("").trigger("blur");
		}
	});
...
function lhcToPostData(){
	...
	console.error(newcode.bonusPropName);
	console.log(code);
	console.log(data);
	$.ajax('/index.php/game/postlhcData', {
		data:{
