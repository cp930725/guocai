...
//			$return['actionNo'] =$date.$return['actionNo'];
		}
...
//		$actionNo = 179*(strtotime(date('Y-m-d', $time))-strtotime('2007-11-11'))/3600/24+$actionNo-3773;
		$sum = 43 * (strtotime(date('Y-m-d', $time))-strtotime('2019-01-26'))/3600/24;
		$actionNo = 179*(strtotime('2019-01-25')-strtotime('2007-11-11'))/3600/24+$sum+$actionNo-3773;
	}
	//北京快乐8
	...
	public function gd11(&$actionNo, &$actionTime, $time=null){
		...
		$actionNo=substr(date('Ymd', $time), 2).$actionNo;
	}
	//江西11选5
	...
	public function sd11(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		if($actionNo>43 || $actionNo<1){
			$time-=24*3600;
		}

		$actionNo=substr(date('Ymd', $time), 2).substr(100+$actionNo,1);
	}
	//上海11选5
	public function sh11(&$actionNo, &$actionTime, $time=null){
		$this->setTimeNo($actionTime, $time);
		if($actionNo>45 || $actionNo<1){
			$time-=24*3600;
		}
		$actionNo=date('Ymd', $time).$actionNo;
	}
