...
class Safe extends WebLoginBase{
	...
	public final function setCBAccount(){
		...
		if(($a!=$b && $a%$b==0)==FALSE) throw new Exception('开户人姓名必须为汉字');
		unset($x);unset($y);unset($a);unset($b);

		// 更新用户信息缓存
		$this->freshSession();
		if(md5($update['coinPassword'])!=$this->user['coinPassword']) throw new Exception('提款密码不正确，请重新输入');
