<?php
	$sql="select u.*, sum(b.mode * b.beiShu * b.actionNum) betAmount, sum(b.bonus) zjAmount, (select sum(c.amount) from blast_member_cash c where c.state=0 and c.`uid`=u.`uid`) cashAmount,(select sum(r.amount) from blast_member_recharge r where r.state in(1,2,9) and r.`uid`=u.`uid`) rechargeAmount from blast_members u left join blast_bets b on u.uid=b.uid and b.isDelete=0 where u.admin=0";
	...
	$sql="select sum(coin) from blast_coin_log where uid=? and liqType between 2 and 3";
