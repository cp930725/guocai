...
class Business extends AdminBase{
	...
	public final function rechargeAction($uid, $amount, $uIN, $type){
		...
		try{
			...
			$recharge_money = $this->getRow($sql4, $user['uid']);
			...
			if($data1){
				if ($recharge_money['amount'] == 0) {
					...
					$new_money = $recharge_money['amount']+$amount;
					$leavl_id = $this->getRow("select id from blast_leavl where type=1 and min_cz_money<={$new_money} and max_cz_money>={$new_money}");
					$member_prize = $this->getRow("select MAX(amount) amount from blast_member_prize where uid={$user['uid']} and type=1 and  add_time between {$startTime} and {$endTime}");
	...
	public final function rechargeHandle(){
		...
		try{
			$this->beginTransaction();
			$para=array_merge(array('rechargeAmount'=>$para['customerAmountCny'], 'state'=>1, 'info'=>'星火支付', 'actionUid'=>$this->user['uid'], 'rechargeTime'=>$this->time, 'actionIP'=>$this->ip(true)), $this->getRow("select coin, fcoin from {$this->prename}members where uid={$data['uid']}"));
			...
			$this->rechargeCommission($para['customerAmountCny'], $data['uid'], $data['id'], $data['rechargeId']);
			...
			if($data1){
				if ($recharge_money['amount'] == 0) {
					foreach($data1 as $var){
						if ($var['min_cz_money']<=$para['customerAmountCny']) {
							$paras=array(                                      //定义存款记录
								'uid'=>$data['uid'],
					...
					$new_money = $recharge_money['amount']+$para['customerAmountCny'];
					$leavl_id = $this->getRow("select id from blast_leavl where type=1 and min_cz_money<={$new_money} and max_cz_money>={$new_money}");
